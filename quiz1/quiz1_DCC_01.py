...
def plot_DCC_1a(save_fig=False, show_fig=False):
    # plotting params
    range_a = (-10, 10)
    range_b = (-10, 10)
    ...
    if save_fig:
        plt.savefig('../plots/quiz/q_DCC_1a_2.png', dpi=300)
    if show_fig:
        plt.show()
...
if __name__ == '__main__':
    ...
    show_axis = False
    path_image = '../rsc/quiz1/q_DCC_1a.png'
    image_shift_xy = (-4.0, -12.0)
    vec_a = np.array((0, -1.0))*90
    vec_b = np.array((1.0, 0.0))*180
    primitive_a = vec_a
    primitive_b = vec_b
    scale_factor = np.linalg.norm(vec_b)/6
